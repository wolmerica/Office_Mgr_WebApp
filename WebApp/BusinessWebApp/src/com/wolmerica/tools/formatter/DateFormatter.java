...
public class DateFormatter extends Formatter {
    ...
    public Object unformat(String target) {
        ...
        try {            
          ...
          date = formatter.parse(digits, new ParsePosition(0));
        }
        catch (IllegalArgumentException iae) {
          throw new FormattingException(PARSE_MSG + iae);
        }
        catch (NullPointerException npe) {
          throw new FormattingException(FORMAT_MSG + npe);            
        }
        catch (Exception e) {
          throw new FormattingException(FORMAT_MSG + e);
        }

        if (date == null)
          throw new FormattingException(FORMAT_MSG + " null value");            
