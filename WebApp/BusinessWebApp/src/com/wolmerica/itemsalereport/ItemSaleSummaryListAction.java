/*
 * ItemSaleSummaryListAction.java
 *
...
public class ItemSaleSummaryListAction extends Action {
  ...
  private ItemSaleListHeadDO getItemSaleSummary(HttpServletRequest request,
                                                Integer customerKey,
                                                Byte mode,
                                                String fromDate,
                                                String toDate)
...
//--------------------------------------------------------------------------------
      String query = "SELECT YEAR(ci.create_stamp) AS ci_year,"
                   ...
                   + "COUNT(*) AS order_qty,"
                   + "SUM(ci.sales_tax) AS sales_tax,"
                   + "SUM(ci.packaging+ci.freight+ci.miscellaneous) AS handling_cost,"
                   + "SUM(ci.item_total) AS item_total "
                   + "FROM customer c,customerinvoice ci "
                   + "WHERE ci.thekey IN (SELECT DISTINCT customerinvoice_key FROM customerinvoiceitem) "
...
//--------------------------------------------------------------------------------
      ...
      while ( rs.next() ) {
        ...
        itemSaleRow.setInvoiceYear(rs.getShort("ci_year"));
        itemSaleRow.setInvoiceMonthName(rs.getString("ci_monthname"));
        itemSaleRow.setOrderQty(rs.getShort("order_qty"));
        itemSaleRow.setSalesTax(rs.getBigDecimal("sales_tax"));
        itemSaleRow.setHandlingCost(rs.getBigDecimal("handling_cost"));
        itemSaleRow.setItemTotal(rs.getBigDecimal("item_total"));

//--------------------------------------------------------------------------------
// Sum up the totals in the header record to be displayed at the end of the list.
//--------------------------------------------------------------------------------
        formHDO.setTransactionTotal(formHDO.getTransactionTotal() + itemSaleRow.getOrderQty());
        formHDO.setSalesTaxTotal(formHDO.getSalesTaxTotal().add(itemSaleRow.getSalesTax()));
        formHDO.setHandlingTotal(formHDO.getHandlingTotal().add(itemSaleRow.getHandlingCost()));
...
//--------------------------------------------------------------------------------
        ItemSaleListHeadDO formHDO = getItemSaleSummary(request,
                                                        customerKey,
                                                        mode,
                                                        fromDate,
                                                        toDate);
