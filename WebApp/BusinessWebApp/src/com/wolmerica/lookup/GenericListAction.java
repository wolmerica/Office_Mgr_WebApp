/*
 * GenericListAction.java
 *
 * Created on March 25, 2009, 08:12 PM
 *
...
public class GenericListAction extends Action {
  ...
  private LookUpListHeadDO getGeneric(HttpServletRequest request,
    ...
    try {
      ...
      String query = "SELECT generic_name, COUNT(*) AS the_count "
                   + "FROM itemdictionary "
      ...
      Integer pageMax;
      if (!jsonId)
        pageMax = new Integer(getPropertyService().getCustomerProperties(request,"generic.list.size"));
      else
      ...
      while ( rs.next() ) {
        ...
        if ((recordCount >= firstRecord) && (recordCount <= lastRecord)) {
          ...
          lookUpRow.setLookUpName(rs.getString("generic_name"));
...
//--------------------------------------------------------------------------------
    ...
    try {
      ...
      cat.debug(this.getClass().getName() + " lookUpNameFilter: " + lookUpNameFilter);      
      ...
      try {
        ...
        LookUpListHeadDO formHDO = getGeneric(request,
                                              lookUpNameFilter,
                                              pageNo,
                                              jsonId);
        if (jsonId) {
