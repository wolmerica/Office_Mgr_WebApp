/*
 * PurchaseOrderItemEditAction.java
 *
 * Created on September 25, 2005, 1:56 PM
 *
...
public class PurchaseOrderItemEditAction extends Action {
  ...
  private void updatePurchaseOrderItem(HttpServletRequest request,
                                         ActionForm form)
   throws Exception, IOException, SQLException {
    ...
    try {
      ...
      PurchaseOrderItemHeadDO formHDO = (PurchaseOrderItemHeadDO) request.getSession().getAttribute("purchaseorderitemHDO");

//--------------------------------------------------------------------------------      
// Prepare a query to update all the purchase order item values but note_line1.
//--------------------------------------------------------------------------------               
      String query = "UPDATE purchaseorderitem SET "
                   + "order_qty=?,"
                   + "update_user= ?,"
...
//--------------------------------------------------------------------------------         
      query = "UPDATE purchaseorderitem SET "
              + "note_line1=? "
              + "WHERE thekey=?";              
      ps2 = conn.prepareStatement(query);

//--------------------------------------------------------------------------------
// Traverse the purchaseorderitem object.
//--------------------------------------------------------------------------------      
      ArrayList poList = formHDO.getPurchaseOrderItemForm();
      PurchaseOrderItemDO formDO = null;
      
//--------------------------------------------------------------------------------         
// Iterate through the rows in the list.
//--------------------------------------------------------------------------------      
      for (int j=0; j < poList.size(); j++) {        
        formDO = (PurchaseOrderItemDO) poList.get(j); 
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
      ...
      updatePurchaseOrderItem(request,form);
    }
    catch (Exception e) {
