 ...
 *
 * Created on August 21, 2006, 09:23 PM
 *
...
public class CategoryListAction extends Action {
  ...
  private LookUpListHeadDO getCategory(HttpServletRequest request,
    ...
    try {
      ...
      String query = "SELECT category, COUNT(*) AS the_count "
      ...
      while ( rs.next() ) {
        ...
        if ((recordCount >= firstRecord) && (recordCount <= lastRecord)) {
          ...
          lookUpRow.setLookUpName(rs.getString("category"));
...
//--------------------------------------------------------------------------------
    ...
    try {
      ...
      try {
        ...
        LookUpListHeadDO formHDO = getCategory(request,
                                               lookUpNameFilter,
                                               pageNo,
                                               jsonId);
        if (jsonId) {
