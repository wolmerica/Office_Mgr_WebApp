...
public class RebateInstanceGetAction extends Action {
  ...
  private RebateInstanceDO buildRebateInstanceForm(HttpServletRequest request,
                                                   Integer riKey)
   throws Exception, SQLException {
    ...
    try {
      ...
      String query = "SELECT rebateinstance.thekey,rebate_key,"
                   ...
                   + "WHERE rebateinstance.thekey=? "
                   + "AND rebate_key = rebate.thekey "
      ...
      if ( rs.next() ) {
        ...
        formDO = new RebateInstanceDO();

        formDO.setKey(riKey);
        formDO.setRebateKey(rs.getInt("rebate_key"));
        ...
        formDO.setPurchaseOrderItemKey(rs.getInt("purchaseorderitem_key"));
        formDO.setTrackingURL(rs.getString("tracking_url"));
        formDO.setNoteLine1(rs.getString("note_line1"));
        formDO.setEligibleId(rs.getBoolean("eligible_id"));
        formDO.setSubmitId(rs.getBoolean("submit_id"));
        formDO.setCompleteId(rs.getBoolean("complete_id"));
        ...
        throw new Exception("RebateInstance " + riKey.toString() + " not found!");
      }
    }
...
//--------------------------------------------------------------------------------
        RebateInstanceDO formDO = buildRebateInstanceForm(request, theKey);
        formDO.setPermissionStatus(usToken);
        request.getSession().setAttribute("rebateinstanceDO", formDO);
        RebateInstanceForm formStr = new RebateInstanceForm();
