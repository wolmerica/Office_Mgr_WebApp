...
public class WorkOrderEditAction extends Action {
  ...
  private void updateWorkOrder(HttpServletRequest request,
                               ActionForm form)
...
//--------------------------------------------------------------------------------
      String query = "UPDATE workorder SET "
                   + "order_qty = ?,"
                   + "the_price = ?,"
                   + "start_stamp = ?,"
                   + "end_stamp = ?,"
...
//--------------------------------------------------------------------------------
      Integer woOrderQty = 0;
      Calendar calValue = Calendar.getInstance();

      for (int j=0; j < woList.size(); j++) {
        formDO = (WorkOrderDO) woList.get(j);
        woOrderQty = formDO.getOrderQty().intValue();

//--------------------------------------------------------------------------------
// Use the startDate, startHour, startMinute to set the startStamp value.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Use the endDate, endHour, endMinute to set the endStamp value.
//--------------------------------------------------------------------------------
        calValue.setTime(formDO.getEndDate());
        calValue.set(calValue.get(Calendar.YEAR),
                     calValue.get(Calendar.MONTH),
                     calValue.get(Calendar.DATE),
                     ...
                     0);
        formDO.setEndStamp(new Timestamp(calValue.getTime().getTime()));
        formDO.getEndStamp().setNanos(0);

//--------------------------------------------------------------------------------
// Perform the normal edit operations for work order.
//--------------------------------------------------------------------------------
        ps.setShort(1, woOrderQty.shortValue());
        ps.setBigDecimal(2, formDO.getEstimatedPrice());
        ps.setTimestamp(3, formDO.getStartStamp());
        ps.setTimestamp(4, formDO.getEndStamp());
