/*
 * CustomerAccountingAddAction.java
 *
...
public class CustomerAccountingAddAction extends Action {
  ...
  private Integer insertCustomerAccounting(HttpServletRequest request,
                                           ActionForm form)
...
//===========================================================================
// Get the maximum key from the customer accounting.
//===========================================================================
      String query = "SELECT COUNT(*) AS ca_cnt, MAX(thekey)+1 AS ca_key "
                   + "FROM customeraccounting";
      ps = conn.prepareStatement(query);
      rs = ps.executeQuery();
      if ( rs.next() ) {
        if ( rs.getInt("ca_cnt") > 0 ) {
           caKey = rs.getInt("ca_key");
        }
      }
      else {
        throw new Exception("Customer Accounting Max() not found!");
      }

      query = "INSERT INTO customeraccounting "
            + "(thekey,customer_key,transactiontype_key,"
      ...
      cat.debug(this.getClass().getName() + ": getKey() = " + caKey);
      cat.debug(this.getClass().getName() + ": customerKey() = " + formDO.getCustomerKey());
...
//--------------------------------------------------------------------------------
      ...
      Integer theKey = null;
      theKey = insertCustomerAccounting(request, form);
