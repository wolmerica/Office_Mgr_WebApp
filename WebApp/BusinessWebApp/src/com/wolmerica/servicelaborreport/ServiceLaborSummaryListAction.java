/*
 * ServiceLaborSummaryListAction.java
 *
 * Created on August 22, 2006, 6:38 PM
 *
...
public class ServiceLaborSummaryListAction extends Action {
  ...
  private ServiceLaborListHeadDO getServiceLaborSummary(HttpServletRequest request,
                                                        Byte mode,
                                                        Integer customerKey,
                                                        String fromDate,
                                                        String toDate)
...
//--------------------------------------------------------------------------------
      query = "SELECT YEAR(ci.create_stamp) AS ci_year,"
            ...
            + "COUNT(*) AS order_qty,"
            + "SUM(cis.order_qty * ROUND(cis.cost_basis,2)) AS labor_total "
            + "FROM customer c,customerinvoice ci, customerinvoiceservice cis "
            + "WHERE ci.thekey = cis.customerinvoice_key "
            + "AND c.thekey = ci.customer_key "
            + "AND NOT ci.active_id";

//--------------------------------------------------------------------------------
// Limit to the following invoice types.
...
//--------------------------------------------------------------------------------
      ...
      while ( rs.next() ) {
        ...
        serviceLaborRow.setInvoiceYear(rs.getShort("ci_year"));
        serviceLaborRow.setInvoiceMonthName(rs.getString("ci_monthname"));
...
//--------------------------------------------------------------------------------
        formHDO.setTransactionTotal(formHDO.getTransactionTotal() + serviceLaborRow.getOrderQty());
...
//--------------------------------------------------------------------------------
// serviceLaborMode is defined as an Byte to use a "switch case" statement
// in the getServiceLabor method.
//--------------------------------------------------------------------------------
      Byte mode = -1;
...
// A value of "ALL" is assigned and evaluated in getServiceLabor()
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
        ServiceLaborListHeadDO formHDO = getServiceLaborSummary(request, mode,
                                                        customerKey,
                                                        fromDate,
                                                        toDate);
