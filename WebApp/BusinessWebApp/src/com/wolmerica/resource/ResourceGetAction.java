...
public class ResourceGetAction extends Action {
  ...
  private ResourceDO buildResourceForm(HttpServletRequest request,
                                       Integer resKey)
   throws Exception, SQLException {
    ...
    try {
      ...
      String query = "SELECT name,"
                   ...
                   + "FROM resource "
                   + "WHERE resource.thekey=?";
      ps = conn.prepareStatement(query);
      ps.setInt(1, resKey);
      rs = ps.executeQuery();
      if ( rs.next() ) {
        formDO = new ResourceDO();

        formDO.setKey(resKey);
        formDO.setResourceName(rs.getString("name"));
        ...
        throw new Exception("Resource " + resKey.toString() + " not found!");
      }
...
//--------------------------------------------------------------------------------
        ResourceDO formDO = buildResourceForm(request, theKey);
        formDO.setPermissionStatus(usToken);
        request.getSession().setAttribute("resource", formDO);
        ResourceForm formStr = new ResourceForm();
