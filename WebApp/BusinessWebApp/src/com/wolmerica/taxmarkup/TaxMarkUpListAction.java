...
public class TaxMarkUpListAction extends Action {
  ...
  private TaxMarkUpListHeadDO getTaxMarkUpList(HttpServletRequest request,
                                                 String taxMarkUpNameFilter,
                                                 Integer pageNo)
    throws Exception, SQLException {

    TaxMarkUpListHeadDO formHDO = new TaxMarkUpListHeadDO();
    ...
    try {
      ...
      String query = "SELECT thekey, precedence, name, description, "
                   + "value, percentage_id, "
                   + "update_user, update_stamp "
                   + "FROM taxmarkup "
                   + "WHERE UPPER(description) LIKE ? "
                   + "ORDER by precedence, name,description ";

      ps = conn.prepareStatement(query);

      ps.setString(1, "%" + taxMarkUpNameFilter.toUpperCase().trim() + "%");
      rs = ps.executeQuery();

      Integer pageMax = new Integer(getPropertyService().getCustomerProperties(request,"taxmarkup.list.size"));
      ...
      while ( rs.next() ) {
        ...
        if ((recordCount >= firstRecord) && (recordCount <= lastRecord)) {
          ...
          taxMarkUpRow.setUpdateUser(rs.getString("update_user"));
          taxMarkUpRow.setUpdateStamp(rs.getTimestamp("update_stamp"));
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
          permissionRows.add(getUserStateService().getUserListToken(request,conn,
                                  this.getClass().getName(),taxMarkUpRow.getKey().intValue()));
      ...
      //===========================================================================
      formHDO.setTaxMarkUpNameFilter(taxMarkUpNameFilter);
      ...
      formHDO.setTaxMarkUpForm(taxMarkUpRows);
...
//--------------------------------------------------------------------------------
    ...
    try {
       String taxMarkUpNameFilter = "";
       if (!(request.getParameter("taxMarkUpNameFilter") == null)) {
         taxMarkUpNameFilter = request.getParameter("taxMarkUpNameFilter");
       }
...
//--------------------------------------------------------------------------------
         TaxMarkUpListHeadDO formHDO = getTaxMarkUpList(request,
                                                        taxMarkUpNameFilter,
                                                        pageNo);
         request.getSession().setAttribute("taxMarkUpListHDO", formHDO);

         // Create the wrapper object for taxMarkUp list.
         TaxMarkUpListHeadForm formHStr = new TaxMarkUpListHeadForm();
