...
public class PdfCanineVacInfoAction extends Action {
...
//--------------------------------------------------------------------------------
        Integer pageNum = 0;
        Integer pageMax = new Integer(getPropertyService().getCustomerProperties(request,"vaccination.canine.page"));
...
//--------------------------------------------------------------------------------
        chunk = new Chunk(getPropertyService().getCustomerProperties(request,"vaccination.canine.title"), FontFactory.getFont(fontType, headerSize, Font.BOLD));
        cell = new PdfPCell(new Phrase(chunk));
        cell.setBorder(0);
        cell.setColspan(3);
        datatable.addCell(cell);
...
//--------------------------------------------------------------------------------
        chunk = new Chunk(getPropertyService().getCustomerProperties(request,"vaccination.canine.date")
...
//--------------------------------------------------------------------------------
      ...
      int headerwidths[] = { 98 }; // percentage
      datatable.setWidths(headerwidths);
...
//--------------------------------------------------------------------------------
      ...
      String messageKey = "vaccination.canine.page" + pageNum.toString() + ".count";
      String message = "";
      ...
      {
        messageKey = "vaccination.canine.page" + pageNum + ".section" + messageCount;
        message = getPropertyService().getCustomerProperties(request,messageKey);
        if (!(message.equalsIgnoreCase(messageKey)))
        {
//--------------------------------------------------------------------------------
// sections with canine vaccination facts.
//--------------------------------------------------------------------------------
          fontStyle = Font.NORMAL;
          if (message.length() >= 3) {  
            if (message.substring(0,3).contains("."))
              fontStyle = Font.BOLD;
          }            
