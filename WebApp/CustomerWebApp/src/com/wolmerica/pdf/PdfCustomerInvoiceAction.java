...
public class PdfCustomerInvoiceAction extends Action {
  ...
  public void doReportGenerator(HttpServletRequest request,
   ...
   throws Exception, IOException, SQLException {
    ...
    String createUser = (String) request.getSession().getAttribute("ACCTNAME");
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
            ciiRow = getCustomerInvoiceItemTotalsByMasterKey(request, conn, ciiRow, ciKey);
...
//--------------------------------------------------------------------------------
  ...
  public CustomerInvoiceItemDO getCustomerInvoiceItemTotalsByMasterKey(HttpServletRequest request,
                                                                       Connection conn,
...
//--------------------------------------------------------------------------------
      ...
      try {
        ...
        int headerwidths[] = { 8,28,34,15,15 }; // percentage
        datatable.setWidths(headerwidths);
        datatable.setWidthPercentage(100); // percentage
...
//--------------------------------------------------------------------------------
// Begin listing the seller name and address to the left.  List invoice details to the right.
//--------------------------------------------------------------------------------
...
// Invoice Total $
//--------------------------------------------------------------------
        chunk = new Chunk(getPropertyService().getCustomerProperties(request,"invoice.report.summary.invoiceTotal"), FontFactory.getFont(fontType, fontSize, Font.BOLD));
        cell = new PdfPCell(new Phrase(chunk));
        cell.setBorder(0);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        datatable.addCell(cell);
        chunk = new Chunk(currencyFormatter.format(invoiceTotal) + " ", FontFactory.getFont(fontType, fontSize , Font.NORMAL));
        cell = new PdfPCell(new Phrase(chunk));
        cell.setBorder(0);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        datatable.addCell(cell);
...
//--------------------------------------------------------------------------------
    ...
    try {
      ...
      if (!(request.getParameter("presentationType") == null)) {
        ...
        if (!(request.getAttribute("presentationType") == null)) {
          theType = new String(request.getAttribute("presentationType").toString());
        }
        else {
          throw new Exception("Request getParameter/getAttribute [presentationType] not found!");
        }
      }
      cat.debug(this.getClass().getName() + ": get[presentationType] = " + theKey.toString());
      request.setAttribute("presentationType", theType);
