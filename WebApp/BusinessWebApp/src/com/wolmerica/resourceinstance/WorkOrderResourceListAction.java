/*
 * WorkOrderResourceListAction.java
 *
 * Created on October 28 10, 2007, 1:43 PM
 *
...
public class WorkOrderResourceListAction extends Action {
  ...
  private ResourceInstanceListHeadForm getWorkOrderResourceList(HttpServletRequest request,
                                                                Integer woKey,
                                                                Integer pageNo)
...
// Initialize to opposite spectrums to make sure an assignment takes place.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Prepare an SQL query to retrieve a few details from the work order.
//--------------------------------------------------------------------------------
      ...
      ScheduleDO formDO = getScheduleService().buildScheduleForm(conn, scheduleKey);

//--------------------------------------------------------------------------------
// Prepare an SQL query to retrieve the assigned work order resources.
//--------------------------------------------------------------------------------
      query = "SELECT resourceinstance.resource_key,"
            + "resource.name AS resource_name "
            + "FROM resourceinstance, resource "
            + "WHERE resourceinstance.workorder_key = ? "
      ...
      while ( rs.next() ) {
        ...
        resourceInstanceRow.setWorkOrderKey(woKey.toString());
...
// between the start and end date.  Return an arrayList.
//--------------------------------------------------------------------------------
        workOrderRows = getScheduleService().getResourceSchedule(request,
                                                        conn,
                                                        woKey,
                                                        rs.getInt("resource_key"),
                                                        startStamp,
                                                        endStamp);
        resourceInstanceRow.setEventCount(new Integer(workOrderRows.size()).toString());
        cat.debug(this.getClass().getName() + ": *[" + rs.getInt("resource_key") + "] workOrderRows size = " + workOrderRows.size());
...
//--------------------------------------------------------------------------------
      query = "SELECT thekey,"
            + "name "
            + "FROM resource "
            + "WHERE thekey NOT IN (SELECT resourceinstance.resource_key "
                                 + "FROM workorder, resourceinstance "
                                 + "WHERE workorder.thekey = ? "
...
//--------------------------------------------------------------------------------
        workOrderRows = getScheduleService().getResourceSchedule(request,
                                                        ...
                                                        endStamp);
        resourceInstanceRow.setEventCount(new Integer(workOrderRows.size()).toString());
        cat.debug(this.getClass().getName() + ": [" + rs.getInt("thekey") + "] workOrderRows size = " + workOrderRows.size());
...
//--------------------------------------------------------------------------------
        ResourceInstanceListHeadForm formHStr = getWorkOrderResourceList(request,
                                                                         theKey,
                                                                         pageNo);
//--------------------------------------------------------------------------------
