...
public class ResourceListAction extends Action {
  ...
  private ResourceListHeadDO getResource(HttpServletRequest request,
...
//--------------------------------------------------------------------------------
// Prepare an SQL query to retrieve the resource records.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
      ...
      Integer pageMax;
      if (!jsonId)
        pageMax = new Integer(getPropertyService().getCustomerProperties(request,"resource.list.size"));
      else 
      ...
      while ( rs.next() ) {
        ...
        if ((recordCount >= firstRecord) && (recordCount <= lastRecord)) {
          ...
          resourceRow.setKey(rs.getInt("thekey"));
          resourceRow.setResourceName(rs.getString("name"));
          resourceRow.setDescription(rs.getString("description"));
          resourceRow.setCreateUser(rs.getString("create_user"));
          resourceRow.setCreateStamp(rs.getTimestamp("create_stamp"));
          resourceRow.setUpdateUser(rs.getString("update_user"));
          resourceRow.setUpdateStamp(rs.getTimestamp("update_stamp"));
...
//--------------------------------------------------------------------------------
            permissionRow = getUserStateService().getUserListToken(request,conn,
                            this.getClass().getName(),resourceRow.getKey());
...
//--------------------------------------------------------------------------------
              resourceRow.setAllowDeleteId(getResourceDependency(conn,
                                                                 sourceTypeKey,
                                                                 resourceRow.getKey()));
            }          
          }
...
//--------------------------------------------------------------------------------
      formHDO.setResourceNameFilter(resourceNameFilter);
...
//--------------------------------------------------------------------------------
  ...
  public Boolean getResourceDependency(Connection conn,
                                       Byte stKey,
                                       Integer sKey)
   throws Exception, SQLException {
...
//--------------------------------------------------------------------------------
      cStmt = conn.prepareCall("{call GetResourceDependency(?,?,?,?)}");
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
      ...
      cat.error(this.getClass().getName() + ": SQLException : " + e.getMessage());
      throw new Exception("setResourceDependency() " + e.getMessage());
    }
    finally
    {
      try
...
//--------------------------------------------------------------------------------
    ...
    try {
      ...
      String resourceNameFilter = "";
      if (request.getParameter("resourceNameFilter") != null) {
        resourceNameFilter = request.getParameter("resourceNameFilter");
      }
...
//--------------------------------------------------------------------------------
      ...
      Byte sourceTypeKey = getUserStateService().getFeatureKey().byteValue();
      
      try {
//--------------------------------------------------------------------------------
// Instantiate a formDO object to populate formStr.
//--------------------------------------------------------------------------------
        ResourceListHeadDO formHDO = getResource(request,
                                                 resourceNameFilter,
                                                 pageNo,
                                                 sourceTypeKey,
                                                 jsonId);
        ...
        if (jsonId) {
          ...
          if (formHDO.getRecordCount() > 0) {
            ArrayList myArray = formHDO.getResourceForm();            
            ResourceDO myDO = new ResourceDO();
            for (int i = 0; i < myArray.size(); i++) {
              myDO = (ResourceDO) myArray.get(i);

              obj = new JSONObject();
              obj.put("id", myDO.getKey());
              obj.put("value", myDO.getResourceName());
              obj.put("info", myDO.getDescription());
              jsonItems.put(obj);
            }
          }
          ...
          request.getSession().setAttribute("resourceListHDO", formHDO);
//--------------------------------------------------------------------------------
