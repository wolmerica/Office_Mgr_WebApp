 ...
 *
 * Created on August 26, 2006, 09:23 PM
 *
...
public class ExpenseListAction extends Action {
  ...
  private LookUpListHeadDO getExpense(HttpServletRequest request,
    ...
    try {
      ...
      Integer pageMax;
      if (!jsonId)
        pageMax = new Integer(getPropertyService().getCustomerProperties(request,"expense.list.size"));
      else 
      ...
      while ( rs.next() ) {
        ...
        if ((recordCount >= firstRecord) && (recordCount <= lastRecord)) {
          ...
          lookUpRow.setLookUpName(rs.getString("name"));
          lookUpRow.setLookUpInfo(rs.getString("category"));
...
//--------------------------------------------------------------------------------
    ...
    try {
      ...
      try {
        ...
        LookUpListHeadDO formHDO = getExpense(request,
                                              lookUpNameFilter,
                                              pageNo,
                                              jsonId);
        if (jsonId) {
