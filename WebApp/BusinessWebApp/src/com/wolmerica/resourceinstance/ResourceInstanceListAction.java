/*
 * ResourceInstanceListAction.java
 *
...
public class ResourceInstanceListAction extends Action {
  ...
  private ResourceInstanceListHeadForm getResourceInstance(HttpServletRequest request,
                                                           Integer resourceKey,
                                                           String fromDate,
                                                           String toDate,
                                                           Integer pageNo)
...
//--------------------------------------------------------------------------------
// Prepare an SQL query to retrieve the resource instance records.
//--------------------------------------------------------------------------------
      String query = "SELECT distinct DATE (workorder.start_stamp) AS start_date "
                   + "FROM resourceinstance, workorder "
                   + "WHERE resourceinstance.resource_key = ? "
                   + "AND resourceinstance.workorder_key = workorder.thekey "              
                   + "AND DATE(workorder.start_stamp) BETWEEN ? AND ? "
                   + "ORDER BY start_date DESC";
      ps = conn.prepareStatement(query);
      ps.setInt(1, resourceKey);
      ps.setDate(2, new java.sql.Date(dtFromDate.getTime()));
...
//          resourceInstanceRow.setKey(rs.getString("thekey"));
//          resourceInstanceRow.setScheduleKey(rs.getString("schedule_key"));
//          resourceInstanceRow.setSubject(rs.getString("subject"));
...
//          resourceInstanceRow.setResourceName(rs.getString("resource_name"));
        ...
        resourceInstanceRow.setStartDate(dateFormatter.formatToWords(rs.getDate("start_date")));
...
//--------------------------------------------------------------------------------
// Create start and end timestamp value for each date.
//--------------------------------------------------------------------------------        
        startCalendar.setTime(rs.getDate("start_date"));
        startCalendar.set(startCalendar.get(Calendar.YEAR),
                     startCalendar.get(Calendar.MONTH),
                     startCalendar.get(Calendar.DATE),
                     0,0,0);
        startStamp = new Timestamp(startCalendar.getTimeInMillis());
        startCalendar.setTime(rs.getDate("start_date"));
        startCalendar.set(startCalendar.get(Calendar.YEAR),
                    startCalendar.get(Calendar.MONTH),
                    startCalendar.get(Calendar.DATE),
                    23,59,59);
        endStamp = new Timestamp(startCalendar.getTimeInMillis());
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------        
        ...
        cat.debug(this.getClass().getName() + ": *[" + resourceKey + "] workOrderRows size = " + workOrderRows.size());
...
//--------------------------------------------------------------------------------
        ResourceInstanceListHeadForm formHStr = getResourceInstance(request,
                                                                    theKey,
                                                                    fromDate,
                                                                    toDate,
                                                                    pageNo);
