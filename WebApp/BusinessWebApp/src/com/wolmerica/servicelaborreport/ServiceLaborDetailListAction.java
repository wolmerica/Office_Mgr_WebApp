/*
 * ServiceLaborDetailListAction.java
 *
 * Created on August 22, 2006, 6:18 PM
 *
...
public class ServiceLaborDetailListAction extends Action {
  ...
  private ServiceLaborListHeadDO getServiceLaborDetail(HttpServletRequest request,
                                                       Byte mode,
                                                       String category,
                                                       Integer customerKey,
                                                       String fromDate,
   ...
   throws Exception, SQLException {
    ...
    ServiceLaborListHeadDO formHDO = new ServiceLaborListHeadDO();
    ServiceLaborDO serviceLaborRow = null;
    ArrayList<ServiceLaborDO> serviceLaborRows = new ArrayList<ServiceLaborDO>();
...
//--------------------------------------------------------------------------------
// Prepare a SQL query to select service Labor details from customer invoice.
//--------------------------------------------------------------------------------
      String query = "SELECT ci.thekey AS ci_key,"
                   ...
                   + "pt.name AS pt_name,"
                   + "cis.order_qty,"
                   + "ROUND(cis.cost_basis,2) AS labor_cost,"
                   + "(cis.order_qty * ROUND(cis.cost_basis,2)) AS labor_total,"
                   + "c.client_name "
...
//--------------------------------------------------------------------------------
// Limit to the reLabor order services.
...
//--------------------------------------------------------------------------------
      ...
      while ( rs.next() ) {
        ++recordCount;
        serviceLaborRow = new ServiceLaborDO();

        serviceLaborRow.setInvoiceKey(rs.getInt("ci_key"));
        serviceLaborRow.setInvoiceDate(rs.getDate("invoice_date"));
        serviceLaborRow.setInvoiceNumber(rs.getString("invoice_num"));
        serviceLaborRow.setServiceName(rs.getString("service_name"));
        serviceLaborRow.setServiceCategory(rs.getString("category_name"));
        serviceLaborRow.setPriceTypeName(rs.getString("pt_name"));
        serviceLaborRow.setOrderQty(rs.getShort("order_qty"));
        serviceLaborRow.setLaborCost(rs.getBigDecimal("labor_cost"));
        serviceLaborRow.setLaborTotal(rs.getBigDecimal("labor_total"));
        clientName = rs.getString("client_name");

        formHDO.setLaborTotal(formHDO.getLaborTotal().add(serviceLaborRow.getLaborTotal()));

        serviceLaborRows.add(serviceLaborRow);
      }
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
      if (serviceLaborRows.isEmpty()) {
        firstRecord = 0;
        serviceLaborRows.add(new ServiceLaborDO());
      }
//--------------------------------------------------------------------------------
// No Pagination logic for serviceLabor.
...
//--------------------------------------------------------------------------------
      ...
      formHDO.setServiceLaborForm(serviceLaborRows);
...
//--------------------------------------------------------------------------------
// itemLaborMode is defined as an Byte to use a "switch case" statement
// in the getItemLabor method.
//--------------------------------------------------------------------------------
      Byte mode = -1;
...
// A value of "ALL" is assigned and evaluated in getItemLabor()
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
      ...
      cat.debug(this.getClass().getName() + " category: " + category);              

      try {
//--------------------------------------------------------------------------------
// Instantiate a formHDO object to populate formHStr.
//--------------------------------------------------------------------------------
        ServiceLaborListHeadDO formHDO = getServiceLaborDetail(request, mode,
                                                    category, customerKey,
                                                    fromDate, toDate);
        request.getSession().setAttribute("webreportHDO", formHDO);

        ServiceLaborListHeadForm formHStr = new ServiceLaborListHeadForm();
